...
#'
#' @description \code{cal.convert.360togreg.grid} converts a 3D field
#' (nx * ny * time) of a 360-days calendar (complete years) to Gregorian
...
#' \code{\link{cal.indices.360togreg.doytoabs()}}. If
#' \code{data.type}=1: filename of RData file containing the
#' fill vector.
#' @param data.type If 0: fill vector is provided as an
#' R object (default). If 1: fill vector is provided
#' in an RData file containing the fill vector only
...
cal.convert.360togreg.grid <- function(org.series,yearstart,yearend,fill.vector,data.type=0){

        # *** check if data.type <= 1 ***
        if (data.type > 1) {
                print(paste('ERROR in cal.convert.360togreg.grid(): data.type is ', data.type,' but has to be either 0 or 1.',sep=''))
                return()
        }
        ...
        if (yearstart > yearend) {
                print(paste('ERROR in cal.convert.360togreg.grid(): Start year (', yearstart,') larger than end year (',yearend,').',sep=''))
                return()
        }

        # *** check if index vector is really a vector ***
        if (!is.vector(indices)) {
                print('ERROR in cal.convert.360togreg.grid(): index vector is not a vector.')
                return()
        }

        # *** check if orginal time series is complete ***
        if (dim(org.series)[3] != (nyears*ndays)) {
                print(paste('ERROR in cal.convert.360togreg.grid(): Length of original timeseries (', dim(org.series)[3],') does not represent complete years (should be ',nyears*360,' according to start and end year.',sep=''))
                return()
        }
